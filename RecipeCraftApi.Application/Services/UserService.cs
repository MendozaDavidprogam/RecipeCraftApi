...
namespace RecipeCraftApi.Application.Services
{
    public class UserService
    {
        ...
        {
            var existing = await _userRepo.GetByEmailAsync(dto.Email ?? string.Empty);
            if (existing != null) throw new Exception("Email already registered");

            var userRole = await _roleRepo.GetByNameAsync("User") ?? throw new Exception("Default role not found");
            var user = new User
            {
                Name = dto.Name,
                Email = dto.Email,
